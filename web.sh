...
sudo openssl req -newkey rsa:2048 -nodes -keyout /etc/pki/tls/private/emy.key -x509 -days 365 -out /etc/pki/tls/certs/emy.crt \
-subj "/C=NG/ST=Lagos/L=Lagos/O=emy/OU=devops/CN=$(curl -s http://169.254.169.254/latest/meta-data/local-hostname)"


sudo sed -i 's/localhost.crt/emy.crt/g'  /etc/httpd/conf.d/ssl.conf

sudo sed -i 's/localhost.key/emy.key/g'  /etc/httpd/conf.d/ssl.conf